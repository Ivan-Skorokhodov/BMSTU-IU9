...
func ScanGraph() *Mealy {
	...
	return &Mealy{count_states, alhabet_size, start_state, transition_matrix, output_matrix}
}
...
func main() {
	...
	new_1 := AufenkampHohn(mealy_1)
	new_canonic_1 := Canonic(*new_1)

	new_2 := AufenkampHohn(mealy_2)
	new_canonic_2 := Canonic(*new_2)

	if new_canonic_1.count_states != new_canonic_2.count_states {
		fmt.Println("NOT EQUAL")
		os.Exit(0)
	}

	if new_canonic_1.alhabet_size != new_canonic_2.alhabet_size {
		fmt.Println("NOT EQUAL")
		os.Exit(0)
	}

	for i := 0; i < new_canonic_1.count_states; i++ {
		for j := 0; j < new_canonic_1.alhabet_size; j++ {
			if new_canonic_1.transition_matrix[i][j] != new_canonic_2.transition_matrix[i][j] {
				fmt.Println("NOT EQUAL")
				os.Exit(0)
			}
		}
	...
	for i := 0; i < new_canonic_1.count_states; i++ {
		for j := 0; j < new_canonic_1.alhabet_size; j++ {
			if new_canonic_1.output_matrix[i][j] != new_canonic_2.output_matrix[i][j] {
				fmt.Println("NOT EQUAL")
				os.Exit(0)
			}
		}
