...
func NewPeer(peerNumber, ip, TCPport, WSPort, nextPeerIP, nextPeerPort string) *Peer {
	return &Peer{
...
func main() {
	if len(os.Args) != 7 {
		fmt.Printf("Usage: %s <peerNumber> <ip> <TCP_port> <WS_port> <next_peer_ip> <next_peer_port>\n", os.Args[0])
		return
	}
	...
	TCPport := os.Args[3]
	WSPort := os.Args[4]
	nextPeerIP := os.Args[5]
	nextPeerPort := os.Args[6]

	peer := NewPeer(peerNumber, ip, TCPport, WSPort, nextPeerIP, nextPeerPort)
	peer.start()
}